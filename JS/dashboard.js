...
window.onload=function(){
  ...
  for (const [key, value] of Object.entries(canvasObj)) {
    ...
    elem.addEventListener("click", e=>{
      ...
      ajaxreq.open('GET', "http://localhost/projects/BankingNew/includes/"+value);
      ajaxreq.send();
...
function submit(){
  ...
  ajaxreq.open("POST", "http://localhost/projects/BankingNew/INCLUDES/debitAppln.php?house="+inputs['house'].value+"&street="+inputs['street'].value+"&district="+inputs['district'].value+"&state="+inputs['state'].value+"&pincode="+inputs['pincode'].value+"&cardtype="+inputs['cardtype'].value, true);
  ajaxreq.send();
      ajaxreq.onreadystatechange = function () {
        if (ajaxreq.readyState == 4 && ajaxreq.status == 200) {
